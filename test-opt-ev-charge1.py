...
for a in [10,20,30,40,50,60]:
    ...
    for b,r in enumerate(ratings):
        ...
        num_iter = len(list(diff.values())[0])
        xtix = range(1,num_iter+1)
        ax4 = fig2.add_subplot(2,2,b+1)
        for h in ev_home:
    ...
    rect = patches.Rectangle((-80.37217, 37.1993), 0.01, 0.005, linewidth=1, 
    ...
    ax1 = sns.boxplot(x="hour", y="voltage", hue="rating",
                 data=df_volt, palette="Set3",ax=ax1)
    ax1.tick_params(axis='y',labelsize=30)
    ax1.tick_params(axis='x',labelsize=30)
    ax1.set_ylabel("Node Voltage",fontsize=30)
    ax1.set_xlabel("Hours",fontsize=30)
    ax1.legend(ncol=4,prop={'size': 30})
    
    fig1.savefig("{}{}.png".format(figpath,str(sub)+"-adoption-"+str(a)),
                bbox_inches='tight')
