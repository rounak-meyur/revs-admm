...
for a in [10,20,30,40,50,60]:
    ...
    leghands = []
    for b,r in enumerate([800, 900, 1000, 1100, 1200, 1300, 1400, 1500, 1600]):
        # Extract data
        prefix = "agentEV-"+str(a)+"-adopt"+str(int(r))+"Watts.txt"
        with open(outpath+prefix,'r') as f:
            lines = f.readlines()
        ...
        bplot = ax.boxplot(volt_data,patch_artist=True,labels=labels)
        ...
        leghands.append(patches.Rectangle((0,0),1,1, facecolor=colors[b],
                               label=str(r)+" Watts"))
        ax.legend(handles=leghands,loc='best',ncol=3,prop={'size': 40})
